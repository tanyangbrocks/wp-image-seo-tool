...
> 🔴 **2026-07-13 已被後續改版取代**：本計畫的「彈窗 + 儲存/取消」互動模型（`<dialog>`、`editableLines` 深複製、Save/Cancel 按鈕）已在介面大改版中整個拿掉，改成「上傳完立即顯示在右欄、即時生效、沒有儲存/取消」的模式；方塊互動也從「點擊+5px 閾值判斷拖曳」改成「移動/縮放把手跟文字編輯區徹底分開」（修正了這個模型本身導致的兩個 bug：打不了字、方塊黏在游標上）。實作細節見 `實作進度.md`「最新完成」與 `js/editor.js`。本文件保留作為當初設計決策的歷史記錄，不再是目前的實際行為。
