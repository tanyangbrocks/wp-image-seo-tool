...
**狀態**：✅ 已修 — 移動把手（`.ovMoveHandle`）取得焦點時，方向鍵可以微調位置（一般 0.5%，按住 Shift 為 2%），符合常見的「方向鍵微調」慣例。縮放把手因為要分辨四個角落方向、邏輯較複雜，這次先不做鍵盤版本（純滑鼠使用者的核心需求已經有拖曳把手，鍵盤使用者至少能移動，缺縮放鍵盤操作視為已知限制而非阻斷性問題）。
...
**狀態**：✅ 已修 — 兩者都補上對應的 `aria-label`（「拖曳移動位置」／「刪除這個文字方塊」，跟現有 `title` 文字一致）。
...
**狀態**：✅ 已修 — 刪除方塊後，如果畫布上還有其他方塊，焦點轉移到「新增文字方塊」按鈕（一個穩定存在、隨時可互動的元素）；如果刪光了，焦點留在畫布容器本身（`tabindex` 已有／新增為可聚焦），避免焦點憑空消失。
